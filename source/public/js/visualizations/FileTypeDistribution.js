...
(function () {
  ...
  FileTypeDistribution.prototype.resize = function () { this._render(); };

  FileTypeDistribution.prototype._render = function () {
    var self = this;
    if (!this.repoId) return;

    this._fetch(this._apiUrl('/api/v1/filetypes')).then(function (data) {
      self.data = data;
      self._draw(data);
    }).catch(function (err) {
  ...
  FileTypeDistribution.prototype._draw = function (data) {
    ...
    var arcs = g.selectAll('.ftd-arc')
      .data(pie(types))
      .enter().append('g').attr('class', 'ftd-arc');

    arcs.append('path')
      .attr('d', arc)
      .attr('fill', function (d) { return color(d.data.extension); })
      ...
      .on('click', function (event, d) {
        window.dispatchEvent(new CustomEvent('viz:filetype-selected', { detail: { extension: d.data.extension } }));
      });
    ...
    arcs.filter(function (d) { return (d.endAngle - d.startAngle) > 0.2; })
      ...
      .attr('text-anchor', 'middle').style('font-size', '11px').style('fill', '#fff')
      .text(function (d) { return d.data.extension; });

    // Center label
    g.append('text').attr('text-anchor', 'middle').attr('dy', '0.35em')
      .style('font-size', '16px').style('font-weight', 'bold')
      .text(d3.sum(types, function (d) { return d.count; }) + ' changes');

    // Legend
    var legend = svg.append('g').attr('transform', 'translate(' + (dims.width - 120) + ', 30)');
    types.slice(0, 12).forEach(function (t, i) {
      legend.append('rect').attr('x', 0).attr('y', i * 18).attr('width', 12).attr('height', 12).attr('fill', color(t.extension));
      legend.append('text').attr('x', 16).attr('y', i * 18 + 10).style('font-size', '10px').text(t.extension + ' (' + t.count + ')');
    });
  };
