...
(function () {
  ...
  FileHotspotTreemap.prototype.resize = function () { this._render(); };

  FileHotspotTreemap.prototype.highlight = function (sel) {
    if (!sel || !sel.file) return;
    d3.select(this.container).selectAll('.tm-cell')
  ...
  FileHotspotTreemap.prototype._render = function () {
    ...
    this._fetch(this._apiUrl('/api/v1/treemap')).then(function (data) {
      self.data = data;
      self._draw(data);
    }).catch(function (err) {
  ...
  FileHotspotTreemap.prototype._draw = function (data) {
    ...
    var hierarchy = d3.hierarchy(data.tree || { name: 'root', children: [] })
      .sum(function (d) { return d.frequency || 0; })
      .sort(function (a, b) { return b.value - a.value; });

    d3.treemap().size([w, h]).padding(2).round(true)(hierarchy);

    var maxFreq = d3.max(hierarchy.leaves(), function (d) { return d.value; }) || 1;
    var color = d3.scaleSequential(d3.interpolateYlOrRd).domain([0, maxFreq]);

    var cells = g.selectAll('.tm-cell')
    ...
    cells.append('rect')
      ...
      .on('mouseover', function (event, d) {
        self._showTooltip(tip, '<b>' + (d.data.path || d.data.name) + '</b><br>Changes: ' + d.value, event);
      })
    ...
    // Title
    svg.append('text').attr('x', dims.width / 2).attr('y', 18)
