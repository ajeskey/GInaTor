...
(function () {
  ...
  BranchMergeGraph.prototype.resize = function () {
    this._render();
  };
  ...
  BranchMergeGraph.prototype._render = function () {
    ...
    this._fetch(this._apiUrl('/api/v1/branches'))
      .then(function (data) {
        self.data = data;
        self._draw(data);
      })
  ...
  BranchMergeGraph.prototype._draw = function (data) {
    ...
    // Time scale
    var dates = commits.map(function (c) {
      return new Date(c.date);
    });
    var xScale = d3
      .scaleTime()
      .domain(
        d3.extent(dates).map(function (d) {
          return d || new Date();
        })
      )
      .range([0, w]);
    ...
    branchNames.forEach(function (name) {
      g.append('text')
        .attr('x', -10)
        .attr('y', laneScale(name) + laneScale.bandwidth() / 2)
    ...
    edges.forEach(function (edge) {
      var src = commits.find(function (c) {
        return c.hash === edge.source;
      });
      var tgt = commits.find(function (c) {
        return c.hash === edge.target;
      });
      if (!src || !tgt) return;
      g.append('line')
    ...
    g.selectAll('.bm-commit')
      ...
      .attr('cy', function (d) {
        return laneScale(d.branch) + laneScale.bandwidth() / 2;
      })
      .attr('r', 5)
      .attr('fill', function (d) {
        return colorScale(d.branch);
      })
      .attr('stroke', '#fff')
      .attr('stroke-width', 1.5)
