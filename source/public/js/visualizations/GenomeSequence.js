...
(function () {
  ...
  GenomeSequence.prototype.resize = function () {
    this._render();
  };
  ...
  GenomeSequence.prototype._render = function () {
    ...
    this._fetch(this._apiUrl('/api/v1/commits', { limit: 500 }))
      .then(function (data) {
        self.data = data;
        self._draw(data);
      })
  ...
  GenomeSequence.prototype._draw = function (data) {
    ...
    var dates = commits.map(function (c) {
      return new Date(c.commitDate || c.date);
    });
    var x = d3
      .scaleTime()
    ...
    g.append('g')
      ...
      .style('font-size', '8px')
      .text(function (d) {
        return d.length > 25 ? '…' + d.slice(-24) : d;
      });
    ...
    g.selectAll('.gs-band')
      ...
      .attr('fill', function (d) {
        return changeColor[d.changeType] || '#999';
      })
      .attr('opacity', 0.8)
