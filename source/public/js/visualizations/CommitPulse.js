...
(function () {
  ...
  CommitPulse.prototype.resize = function () { this._render(); };
  ...
  CommitPulse.prototype._draw = function (data) {
    ...
    var x = d3.scaleTime()
      .domain(d3.extent(points, function (d) { return d.date; }) || [new Date(), new Date()])
      .range([0, w]);

    var y = d3.scaleLinear()
      .domain([0, d3.max(points, function (d) { return d.count; }) || 1])
      .nice().range([h, 0]);
    ...
    g.append('path').datum(points)
      .attr('fill', '#3b82f6').attr('opacity', 0.1).attr('d', area);
    ...
    g.selectAll('.cp-dot')
      ...
      .attr('r', function (d) { return d.spike ? 6 : 3; })
      .attr('fill', function (d) { return d.spike ? '#ef4444' : '#3b82f6'; })
      .attr('stroke', function (d) { return d.spike ? '#dc2626' : 'none'; })
      .attr('stroke-width', function (d) { return d.spike ? 2 : 0; })
      .on('mouseover', function (event, d) {
        self._showTooltip(tip,
          '<b>' + d.date.toLocaleDateString() + '</b><br>Commits: ' + d.count +
    ...
    // Y label
    g.append('text').attr('transform', 'rotate(-90)').attr('y', -40).attr('x', -h / 2)
      .attr('text-anchor', 'middle').style('font-size', '11px').text('Commits');
  };
