...
(function () {
  ...
  OwnershipSunburst.prototype.resize = function () {
    this._render();
  };
  ...
  OwnershipSunburst.prototype.highlight = function (sel) {
    ...
    d3.select(this.container)
      ...
      .classed('opacity-30', function (d) {
        return d.data.primaryContributor !== sel.author;
      });
  };
  ...
  OwnershipSunburst.prototype._render = function () {
    ...
    this._fetch(this._apiUrl('/api/v1/sunburst'))
      .then(function (data) {
        self.data = data;
        self._draw(data);
      })
      ...
      .on('click', function (event, d) {
        ...
        g.selectAll('.sb-arc')
          ...
          .attrTween('d', function (node) {
            var _xi = d3.interpolate(
              node.x0,
              Math.max(0, Math.min(2 * Math.PI, ((node.x0 - d.x0) / (d.x1 - d.x0)) * 2 * Math.PI))
            );
            return function () {
              return arc(node);
            };
